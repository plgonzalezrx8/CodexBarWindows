...
public class CursorProvider : IProviderProbe
{
    ...
    public async Task<ProviderUsageStatus> FetchStatusAsync(CancellationToken cancellationToken)
    {
        ...
        var cached = _credentialManager.GetCachedCookieHeader("cursor");
        if (cached != null && !string.IsNullOrWhiteSpace(cached.CookieHeader))
        {
            ...
            {
                _credentialManager.ClearCachedCookieHeader("cursor");
            }
        }
        ...
        {
            var cookieHeader = _cookieService.GetCookieHeader(domain);
            ...
            try
            {
                ...
                _credentialManager.CacheCookieHeader("cursor", cookieHeader, "browser-auto");
                return status;
            }
            catch (CursorAuthException)
            {
        ...
        var manualCookie = _credentialManager.GetCredential("cursor", "manual-cookie");
    ...
    private static async Task<(CursorUsageSummary Summary, string RawJson)> FetchUsageSummary(
        string cookieHeader, CancellationToken ct)
    {
    ...
    private static async Task<CursorUserInfo> FetchUserInfo(string cookieHeader, CancellationToken ct)
    {
