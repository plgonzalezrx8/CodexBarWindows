...
public class JetBrainsProvider : IProviderProbe
{
    ...
    public JetBrainsProvider(SettingsService settings, IEnvironmentService environmentService)
    {
        _settings = settings;
    ...
    private JetBrainsIDE? DetectLatestIDE()
    {
    ...
    private static ProviderUsageStatus ParseQuotaFile(string path, JetBrainsIDE ide)
    {
        ...
        return ParseQuotaJson(decoded, nextRefillRaw, ide.DisplayName);
    }

    internal static ProviderUsageStatus ParseQuotaJson(string json, string? refillJson, string displayName)
    {
        ...
        var tooltipParts = new List<string> { $"JetBrains AI ({displayName})" };
