...
public class KiroProvider : IProviderProbe
{
    ...
    public KiroProvider(ICommandRunner commandRunner, SettingsService settings)
    {
    ...
    public async Task<ProviderUsageStatus> FetchStatusAsync(CancellationToken ct)
    {
        ...
        var versionResult = await _commandRunner.ExecuteCommandAsync("kiro-cli", "--version", timeoutMilliseconds: 5000, cancellationToken: ct);
        if (versionResult.ExitCode != 0)
            return MakeError("kiro-cli not found. Install it from https://kiro.dev");

        // 2. Check login
        var whoamiResult = await _commandRunner.ExecuteCommandAsync("kiro-cli", "whoami", timeoutMilliseconds: 5000, cancellationToken: ct);
        ...
        // 3. Fetch usage
        var usageResult = await _commandRunner.ExecuteCommandAsync(
            "kiro-cli",
    ...
    internal static ProviderUsageStatus ParseUsageOutput(string rawOutput)
    {
