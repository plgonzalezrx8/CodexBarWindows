...
public class CodexProvider : IProviderProbe
{
    ...
    public CodexProvider(SettingsService settings, IEnvironmentService environmentService, HttpClient httpClient)
    {
        _settings = settings;
    ...
    internal static ProviderUsageStatus CreateStatus(CodexUsageSnapshot usage)
    {
    ...
    private async Task<CodexUsageSnapshot> FetchUsageAsync(CodexCredentials credentials, CancellationToken cancellationToken)
    {
        ...
        return ParseUsageSnapshot(root);
    }

    private async Task<CodexCredentials> RefreshCredentialsAsync(CodexCredentials credentials, CancellationToken cancellationToken)
    {
    ...
    private string GetCodexHomePath()
    {
        var codexHome = _environmentService.GetEnvironmentVariable("CODEX_HOME");
        if (!string.IsNullOrWhiteSpace(codexHome))
        {
            return codexHome;
        }

        return Path.Combine(_environmentService.GetFolderPath(Environment.SpecialFolder.UserProfile), ".codex");
    }

    internal static CodexUsageSnapshot ParseUsageSnapshot(JsonElement root)
    {
    ...
    internal sealed record CodexUsageSnapshot(double SessionProgress, double WeeklyProgress, double? Credits, string? SessionReset, string? WeeklyReset);
